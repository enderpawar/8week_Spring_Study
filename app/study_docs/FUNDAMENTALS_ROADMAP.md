...
**현재 확인 시점: 2026-09-25 — Week C D4(연관관계 + Hibernate LAZY 프록시) 진행 중, 미완료.** Full 루프 ①개념+완성예제·⑤예측→실행→차이설명 완료, ②빈칸예제(제시만 됨)·③독립 변형·④인출(노트 덮고 재작성)·⑥복습큐 등록(일부)은 남음. **다음 세션은 Day18 이어서(빈칸 예제부터) 재개** — Week C D4를 아직 `[x]`로 표시하지 않는다. 상세는 [Day18 진행 기록](days/WeekC/Day18_0925/progress.md).
