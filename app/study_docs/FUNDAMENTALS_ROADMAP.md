...
│  ├─ …
│  └─ Day35_0828/vocab.md, quiz.md, explain-log.md, velog_post.md   ← Week E D7
│     (progress.md는 진행 상태·부채를 남길 필요가 있는 날만 추가)
├─ velog/                      ← 주 1편 심화글. 여러 Day를 하나의 기술적 질문으로 묶는다.
├─ interview-notes.md          ← 면접 문장(기존 파일 유지, 누적)
├─ spring-core-notes.md        ← Week A~C 인출·되설명 워크시트(기존 파일 재사용)
├─ 기술부채.md                  ← 예약된 부채 원장(부채·분류·해결 Day·상태) — 단일 추적 위치.
...
각 Day의 **완료 판정 산출물**은 `vocab.md`, `quiz.md`, `explain-log.md`이며, 진행 상태나 기술 부채를 별도로 남길 필요가 있을 때만 `progress.md`를 작성한다. Day 완료는 코드·테스트와 이 산출물로 판정하며, **`velog_post.md`는 Day별로 계속 쓰되 완료 조건으로는 사용하지 않는다**(위 「세션 재개용 진행 체크리스트」의 `[x]` 기준과 동일). 글이 밀렸다는 이유로 다음 Day 진도를 막지 않고, 다음 세션에 이어서 쓴다.

`velog_post.md`는 [Spring 기본기 기술 블로그 템플릿](VELOg_POST_TEMPLATE.md)을 따르며, 핵심 메커니즘 또는 설계 판단과 실제 검증 근거를 반드시 포함한다. 주 1편으로 승격하는 심화글은 여러 Day를 하나의 기술적 질문으로 묶어 `study_docs/velog/`에 보관한다.
...
- **Velog 글은 매일 쓰되 분량을 소재에 맞춘다.** 연속된 공개 기록 자체가 성실성의 증거이므로 Day별로 계속 쓴다. 다만 **모든 날에 같은 분량을 요구하지 않는다** — Light 소재는 40~60줄, Full 소재는 90~110줄이며, 근거가 없는 절은 채우지 말고 생략한다. 얕은 소재를 억지로 늘린 글 한 편이 시리즈 전체의 인상을 결정한다. **주 1편은 그 주에서 가장 좋은 주제를 골라 `Day N` 접두사 없는 심화글로 승격**해 여러 Day를 하나의 기술적 질문으로 묶는다. 글 앞부분에서 핵심 메커니즘과 설계 판단을 설명하고, 실제 학습자의 예측·실행 결과·검증 근거만 사용한다. 예측과 오답은 하나의 교정 서사로 합치며, 실제로 비교하지 않은 대안이나 수행하지 않은 검증을 만들지 않는다. 인출은 글을 읽기 **전에** `quiz.md`와 `복습큐.md`에서 수행하고, Velog에는 필요할 때만 정답 없는 면접 질문을 1~2개 남긴다. `[직접 작성]`은 덮어쓰지 않는다.
- **기술 부채는 분류해서 원장 한 곳에 모은다.** 즉시 차단(다음 실험 결과를 왜곡·빌드 실패·보안 위험) / 예약된 부채(해결 Day 지정) / 범위 경계(이 트랙에서 해결하지 않음, 1회만 명시) 중 하나로 나눠 `기술부채.md`에 기록한다. 같은 한계를 매 글마다 반복해 적지 않는다.
- **각 주 D7은 지연 흡수와 부채 점검 슬롯이다.** ① 그 주의 미완료 필수 유닛 처리 → ② 즉시 차단이거나 현재까지 배운 범위로 해결 가능한 고우선순위 부채 상환 → ③ 주간 독립과제 순으로 쓴다. 이후 주차의 개념이 필요한 부채는 해결 Day만 배정하고, **부채가 남아 있다는 이유만으로 선행 진도를 중단하지 않는다.**
