# [백엔드 기본기 Day7] 1주차 버퍼 — 저장 계약과 Absence of Value의 오류 경계

Week A D7은 새 기능을 추가하는 날이 아니라 Day03·04에서 **바로 고칠 것**으로 미뤄 둔 기술부채를 갚는 버퍼였다. 계획은 7월 31일이었지만 D6 누적시험과 함께 8월 2일에 진행했다. 기존 예약을 다시 `save()`하면 같은 객체 참조가 목록에 한 칸 더 들어갈 수 있었고, 없는 번호를 조회하면 `null`이 Service까지 흘러갔으며, 예상 밖 예외의 메시지가 500 본문에 그대로 실렸다.
...
|---|---|---|
| 저장 계약 | 신규·기존 객체를 저장할 때의 동작 약속 | `InMemoryReservationRepository.save()`의 `add` / `set` 분기 |
| `Optional` | 값의 부재 가능성을 반환형에 드러내는 컨테이너 | `Optional<Reservation> findById(Long id)` |
| 도메인 예외 | 업무 의미가 있는 실패를 나타내는 예외 | `ReservationNotFoundException` |
| 메서드 검증 | Controller 인수에 직접 선언한 제약을 검사하는 MVC 처리 | `@PathVariable @Positive Long id` |
| 오류 정보 분리 | 내부 원인과 외부 응답에 공개할 정보를 나누는 것 | `handleUnexpected()`의 `log.error(..., ex)`와 일반 500 본문 |
| MockMvc | 서버 포트를 열지 않고 Spring MVC 요청 흐름을 검증하는 도구 | `ReservationControllerHttpTest` |
...
Day04 시점의 `save()`는 id가 없으면 번호만 부여하고, id 유무와 관계없이 `store.add(reservation)`을 호출했다. 이 코드에서 취소 흐름을 따라가면 문제가 보인다.
...
`Reservation`은 가변 객체이고, `ArrayList`에는 복사본이 아니라 참조가 들어간다. 그래서 `r.cancel()`을 호출하는 순간 상태 변경은 이미 끝났다. 그 뒤의 `add()`는 새 값을 저장하는 동작이 아니라 **같은 참조를 한 칸 더 넣는 동작**이 된다. `ArrayList`는 같은 참조의 중복 삽입을 막지 않는다.
...
보장하는 것은 "저장 후에도 목록 크기가 예약 수와 같고, 같은 id가 두 번 들어가지 않는다"는 불변식이다. 지금 구조에서 `set()`은 이미 변경된 같은 참조를 같은 자리에 다시 넣으므로 메모리상으로는 사실상 변화가 없다. 그래도 계약을 분리해 두면 저장소 구현이 DB로 바뀌었을 때 "기존 id는 갱신"이라는 약속이 그대로 유지된다.

보장하지 않는 것도 있다. `ArrayList`와 `nextId`는 여러 스레드의 동시 접근에 안전하지 않고, 데이터는 프로세스 재시작 후 사라진다. CS로는 자료구조의 삽입과 갱신 구분이며, DB에서 PK로 INSERT 대상과 UPDATE 대상을 구분하는 문제와 연결된다.
...
`null` 반환은 메서드 선언 `Reservation findById(Long id)`만 봐서는 알 수 없는 숨은 경우다. 호출자가 확인을 빠뜨리면 실패는 원인에서 먼 곳에서 `NullPointerException`으로 드러난다. 당시 이 경로를 실제 요청으로 실행한 기록은 없으므로 NPE가 어떤 응답으로 끝났는지는 **미검증**이다.
...
`Optional`이 오류 처리를 끝내 주지는 않는다. 값이 없다는 사실만 드러낼 뿐, 그것이 업무상 어떤 실패인지는 Service가 정한다. 이 프로젝트에서는 "해당 예약 없음"이라는 의미를 `ReservationNotFoundException`에 담았다. 이 예외는 `RuntimeException`을 상속하므로 `cancel()` 시그니처에 `throws`를 추가하지 않는다.
...
Repository는 HTTP를 모르고, Service는 JSON을 모른다. 웹 경계로의 변환은 Spring MVC가 수행한다. 예외가 Controller 밖으로 나가면 `DispatcherServlet`이 예외 리졸버에 처리를 맡기고, 리졸버는 `@RestControllerAdvice` Bean인 `GlobalExceptionHandler`에서 예외 타입에 맞는 `@ExceptionHandler`를 찾는다.
...
MockMvc는 네트워크와 서블릿 컨테이너를 거치지 않으므로 실제 포트를 여는 테스트와 같지는 않다. 대신 Day04에서 컴파일과 테스트가 모두 성공한 뒤 실제 요청에서만 드러난 `@PathVariable` 불일치 같은 문제를 테스트 단계에서 잡을 수 있다.
...
전체 `./gradlew.bat test`에서 10개 테스트가 모두 통과했다. 수정 전 코드를 대상으로 실패 테스트를 먼저 실행한 기록은 없으므로 TDD의 red-green 순서를 수행했다고 주장하지 않는다.
